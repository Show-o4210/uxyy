...
对照测试表明，完全同时启动大量任务会显著增加 `rewards=[]`。批量任务仍保留 20 的并发上限，但现在将不同任务组错开 100ms；每个任务内部的两个请求连续执行，不额外停顿，也不拆成两个批次。

在相同的 20 并发上限、每档 20 个任务的单轮筛选测试中：

| 发送时序 | 确认成功 | 竞争落败 | 真实失败 | 总耗时 | 确认成功吞吐 |
|---|---:|---:|---:|---:|---:|
| 完全连续并发（组间 0ms） | 1 | 19 | 0 | 4.102 秒 | 0.244 次/秒 |
| 成对连续、组间 100ms | 6 | 14 | 0 | 6.504 秒 | 0.923 次/秒 |
| 成对发送、组间 100ms、组内等待 25ms | 6 | 14 | 0 | 7.169 秒 | 0.837 次/秒 |
| 成对发送、组间 100ms、组内等待 50ms | 7 | 13 | 0 | 10.256 秒 | 0.683 次/秒 |

按单位时间的确认成功吞吐计算，“组内连续、组间 100ms”在本轮中最高：成功率由 5% 提升到 30%，吞吐由 0.244 次/秒提升到 0.923 次/秒，约为完全连续并发的 3.79 倍。组内等待 50ms 虽将成功数提高到 7，但总耗时增至 10.256 秒，单位时间效率反而下降。因此默认采用“组内连续、组间 100ms”。单轮结果用于选择调度参数，不代表不同运行条件下的固定成功率。
